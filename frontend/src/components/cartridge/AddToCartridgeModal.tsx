...
interface Props {
  open: boolean;
  onClose: () => void;
  /** The stash we're adding to. We mutate its items via updateCartridge. */
  stashId: string;
...
interface PickerRow {
  ...
  spec: CartridgeItemSpec;
}

export default function AddToCartridgeModal({
  open,
  onClose,
  stashId,
  workspaceId,
  ...
  function nextItemsFor(newItems: CartridgeItemSpec[]): CartridgeItemSpec[] {
    return [
  ...
  async function addNewItems(newItems: CartridgeItemSpec[]) {
    await updateCartridge(stashId, { items: nextItemsFor(newItems) });
    onAdded();
    onClose();
  }
  ...
  async function attachExternalCartridge() {
    ...
    setError(null);
    try {
      await addExternalCartridge(workspaceId, slug);
      onAdded();
      onClose();
    } catch (e) {
  ...
  function submitCurrentMode() {
    ...
    return attachExternalCartridge();
  }
        ...
        {mode === "existing" ? (
          ...
          <div className="px-4 py-4">
            ...
            <input
              ...
              placeholder="https://app.joinstash.ai/cartridges/example-slug"
              className="mt-3 w-full rounded-md border border-border bg-surface px-2.5 py-1.5 text-[13px] text-foreground placeholder:text-muted focus:border-[var(--color-brand-400)] focus:outline-none"
            />
          </div>
        )}
