...
import { loadPublicSkillPreview } from "@/lib/skillPreviewFetch";
import {
...
export async function GET(request: NextRequest) {
  ...
  if (!data) return new Response("Skill not found", { status: 404 });
...
function PreviewImage({ card }: { card: PreviewCard }) {
  const lines =
    card.lines.length > 0
      ? card.lines
      : [{ label: "Contents", meta: "Skill", excerpt: card.description }];
  ...
  const sideMeta =
    card.kind === "skill"
      ? card.stats.slice(1, 4).join(" - ") || "Shared skill"
      : "1";
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {truncate(card.kind === "skill" ? card.title : card.skillTitle, 28)}
        </div>
        <div
          style={{
            width: 650,
        ...
        >
          ...
          <span style={{ display: "flex", marginLeft: 12 }}>
            Search {truncate(card.skillTitle, 44)}
          </span>
        </div>
...
function KindIcon({ kind, size }: { kind: PreviewCard["kind"]; size: number }) {
  ...
  if (kind === "skill") return <SkillGlyph />;
...
function editedLabel(kind: PreviewCard["kind"], value: string | null): string {
  ...
  const label = kind === "skill" ? "Updated" : "Last edited";
