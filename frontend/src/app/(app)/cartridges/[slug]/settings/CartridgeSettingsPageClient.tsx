...
export default function CartridgeSettingsPageClient({ slug }: { slug: string }) {
  ...
  useBreadcrumbs(
    [
      ...
      { label: stash?.title ?? "Stash", href: `/cartridges/${slug}` },
      { label: "Settings" },
    ],
    `stash-settings/${slug}/${stash?.id ?? "loading"}`
  );
  useActiveWorkspaceId(stash?.workspace_id ?? null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const nextData = await getPublicCartridge(slug);
      setData(nextData);
    } catch (e) {
      setData(null);
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      router.replace(`/login?next=${encodeURIComponent(`/cartridges/${slug}/settings`)}`);
    }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      resetStashNavigationCache();
      router.push(`/workspaces/${stash.workspace_id}/cartridges`);
    } catch (e) {
