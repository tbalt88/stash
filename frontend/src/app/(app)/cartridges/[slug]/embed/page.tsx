...
async function loadCartridge(slug: string): Promise<PublicCartridge | null> {
...
export default async function CartridgeEmbed({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const data = await loadCartridge(slug);
  if (!data) notFound();
  return (
    <main className="px-4 py-4">
      ...
      <p className="mt-4 text-right font-mono text-[10px] uppercase tracking-wider text-muted">
        <a href={`/cartridges/${slug}`} target="_blank" rel="noreferrer" className="hover:text-ink">
          on Stash ↗
        </a>
      </p>
    </main>
...
function ItemBody({ item }: { item: CartridgeItemInlined }) {
