...
@pytest.mark.asyncio
async def test_public_cartridge_text_is_agent_homepage(client: AsyncClient):
    ...
    assert f"http://localhost:3457/cartridges/{slug}.md" in text
    assert f"http://localhost:3457/cartridges/{slug}.json" in text
    assert f"/cartridges/{slug}/items/page/{page['id']}.md" in text
    ...
    assert f"stash read http://localhost:3457/cartridges/{slug}" in text


@pytest.mark.asyncio
async def test_public_cartridge_item_text_strips_html_page_content(client: AsyncClient):
    api_key, _ = await _register(client)
    workspace = await _create_workspace(client, api_key)
...
@pytest.mark.asyncio
async def test_llms_txt_documents_agent_cartridge_reads(client: AsyncClient):
    resp = await client.get("/llms.txt")
    assert resp.status_code == 200
    assert "Public Stash URLs are agent-readable" in resp.text
    assert "/cartridges/example.md" in resp.text
    assert "## Connect This Repo To Stash" in resp.text
    ...
    assert "stash read https://app.joinstash.ai/cartridges/example" in resp.text


@pytest.mark.asyncio
async def test_cartridge_skill_manifest_includes_agent_install_pitch(client: AsyncClient):
