"""Cartridges — the privacy and sharing boundary for workspace resources."""
...
async def _attach_items(stash: dict) -> dict:
    ...
    rows = await pool.fetch(
        ...
        "FROM cartridge_items WHERE cartridge_id = $1 ORDER BY position, object_type, object_id",
        stash["id"],
    )
...
def _mark_native(stash: dict) -> dict:
    if stash.get("forked_from_cartridge_id"):
...
async def _replace_items(conn, cartridge_id: UUID, items: list) -> None:
    await conn.execute("DELETE FROM cartridge_items WHERE cartridge_id = $1", cartridge_id)
    for i, item in enumerate(items):
        await conn.execute(
            "INSERT INTO cartridge_items (cartridge_id, object_type, object_id, position, label_override) "
            "VALUES ($1, $2, $3, $4, $5)",
            cartridge_id,
    ...
    for item in items:
        ...
        for row in rows:
            if access == "private" and row["access"] != "private":
                raise ValueError(
                    "Private Cartridges can only include items that are not in workspace or public Cartridges"
                )
            if access != "private" and row["access"] == "private":
                raise ValueError(
                    "Items in private Cartridges cannot be added to workspace or public Cartridges"
                )
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            inserted = await conn.fetchrow(
                "INSERT INTO cartridges (workspace_id, slug, title, description, owner_id, "
...
async def update_cartridge(
    ...
    if not stash or not await user_can_manage(cartridge_id, user_id):
        return None
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            if partition_items is not None:
                await _validate_item_partition(conn, next_access, partition_items, cartridge_id)
            if items is not None:
                sets.append("updated_at = now()")
            if sets:
                args.append(cartridge_id)
                await conn.execute(
                    f"UPDATE cartridges SET {', '.join(sets)} WHERE id = ${idx}",
                    *args,
                )
            if items is not None:
                await _replace_items(conn, cartridge_id, items)

    row = await pool.fetchrow(f"{_STASH_SELECT} WHERE v.id = $1", cartridge_id)
    return await _attach_items(dict(row))


async def add_sessions_to_cartridge(
    *,
    cartridge_id: UUID,
    workspace_id: UUID,
    user_id: UUID,
    ...
    if not session_ids:
        return
    if not await user_can_manage(cartridge_id, user_id):
        raise ValueError("Not allowed to manage this Stash")

    pool = get_pool()
    stash = await pool.fetchrow(
        "SELECT id, workspace_id FROM cartridges WHERE id = $1",
        cartridge_id,
    )
    ...
    start_position = await pool.fetchval(
        "SELECT COALESCE(MAX(position), -1) + 1 FROM cartridge_items WHERE cartridge_id = $1",
        cartridge_id,
    )
    async with pool.acquire() as conn:
        async with conn.transaction():
            for offset, row in enumerate(rows):
                await conn.execute(
                    "INSERT INTO cartridge_items "
                    "(cartridge_id, object_type, object_id, position, label_override) "
                    "VALUES ($1, 'session', $2, $3, $4) "
                    "ON CONFLICT (cartridge_id, object_type, object_id) DO NOTHING",
                    cartridge_id,
                    row["id"],
...
async def delete_cartridge(cartridge_id: UUID, user_id: UUID) -> bool:
    pool = get_pool()
    if not await user_can_manage(cartridge_id, user_id):
        return False
    result = await pool.execute("DELETE FROM cartridges WHERE id = $1", cartridge_id)
...
async def list_public_stashes(
    *,
    ...
    """Catalog of Cartridges whose every underlying item is anonymously readable."""
    pool = get_pool()
    ...
    rows = await pool.fetch(
        ...
        f"FROM cartridges v JOIN users u ON u.id = v.owner_id "
...
async def _filter_readable_stashes(cartridges: list[dict], user_id: UUID | None) -> list[dict]:
    readable = []
...
async def _fork_page(
    ...
    *,
    ...
    metadata.pop("shared_in_cartridge_id", None)
...
async def add_external_cartridge(workspace_id: UUID, slug: str, added_by: UUID) -> dict | None:
    pool = get_pool()
    ...
    existing = await pool.fetchrow(
        f"{_STASH_SELECT} WHERE v.workspace_id = $1 AND v.forked_from_cartridge_id = $2",
        workspace_id,
        stash["id"],
    )
    if existing:
        ...
        await cartridge_invite_service.mark_invite_accepted_for_cartridge(
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            inserted = await conn.fetchrow(
                ...
                "cover_image_url, icon_url, forked_from_cartridge_id) "
...
async def remove_external_cartridge(workspace_id: UUID, cartridge_id: UUID, user_id: UUID) -> bool:
    pool = get_pool()
    stash = await pool.fetchrow(
        "SELECT id FROM cartridges "
        "WHERE workspace_id = $1 AND id = $2 AND forked_from_cartridge_id IS NOT NULL",
        workspace_id,
        cartridge_id,
    )
    if not stash or not await user_can_manage(cartridge_id, user_id):
...
async def list_object_stashes(
    ...
    for target_type, target_id in await _containing_targets(pool, object_type, object_id):
        target_rows = await pool.fetch(
            ...
            "JOIN cartridge_items vi ON vi.cartridge_id = v.id "
    ...
    cartridges = sorted(deduped.values(), key=lambda row: row["updated_at"], reverse=True)
    return await _filter_readable_stashes([await _attach_items(row) for row in cartridges], user_id)


async def get_cartridge(cartridge_id: UUID) -> dict | None:
    pool = get_pool()
    row = await pool.fetchrow(f"{_STASH_SELECT} WHERE v.id = $1", cartridge_id)
    return await _attach_items(dict(row)) if row else None


async def get_public_cartridge(slug: str, viewer_id: UUID | None = None) -> dict | None:
    """Resolve a Stash by slug for the given viewer (None = anonymous).

    Cartridges are the privacy boundary: public Cartridges render anonymously,
    workspace Cartridges render for workspace members, and private Cartridges render
...
async def create_shared_page(
    ...
    *,
    ...
    page = await files_tree_service.create_page(
        ...
        folder_id=None,
        content=content,
        metadata={"shared_in_cartridge_id": str(cartridge_id)},
...
def _agent_item_url(base_url: str, stash: dict, item: dict, suffix: str) -> str:
    return (
        f"{base_url}/cartridges/{stash['slug']}/items/"
...
def stash_to_text(stash: dict, workspace_name: str, items: list[dict], base_url: str) -> str:
    ...
    parts.append(
        ...
        f"- Markdown homepage: {base_url}/cartridges/{stash['slug']}.md\n"
        f"- Structured JSON: {base_url}/cartridges/{stash['slug']}.json\n"
    ...
    parts.append(agent_install_pitch(f"{base_url}/cartridges/{stash['slug']}"))
...
def item_to_text(stash: dict, item: dict, base_url: str) -> str:
    ...
    parts = [
        f"# {label}",
        f"Stash: [{stash['title']}]({base_url}/cartridges/{stash['slug']}.md)",
        "## Agent Navigation\n\n"
        f"- Back to Stash homepage: {base_url}/cartridges/{stash['slug']}.md\n"
