// Brand icons for the data-source integrations, mirrored from the product's
// frontend/src/components/integrations/BrandIcons.tsx so the marketing site
// and the app show the same marks.
...
export type Integration = { provider: string; name: string; blurb: string };

// The data sources agents can connect, in the order we show them.
export const INTEGRATIONS: Integration[] = [
  { provider: "github", name: "GitHub", blurb: "Pick repos your agent can navigate like a filesystem." },
  { provider: "google", name: "Google Drive", blurb: "Index My Drive and read docs on demand." },
  { provider: "gmail", name: "Gmail", blurb: "Search messages and read email on demand." },
  { provider: "notion", name: "Notion", blurb: "Pages and databases you share with Stash." },
  { provider: "slack", name: "Slack", blurb: "Channel history, kept in sync." },
  { provider: "granola", name: "Granola", blurb: "Meeting notes and transcripts." },
...
function GitHubIcon({ size = defaultSize }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className="text-ink" aria-hidden>
...
function GoogleDriveIcon({ size = defaultSize }: Props) {
  return (
...
function GmailIcon({ size = defaultSize }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 256 193" aria-hidden>
...
function NotionIcon({ size = defaultSize }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className="text-ink" aria-hidden>
...
function SlackIcon({ size = defaultSize }: Props) {
  return (
...
function GranolaIcon({ size = defaultSize }: Props) {
  return (
    // eslint-disable-next-line @next/next/no-img-element
    <img src="/granola-logo.png" alt="" width={size} height={size} className="rounded-[5px]" aria-hidden />
  );
}

function JiraIcon({ size = defaultSize }: Props) {
  return (
...
function AsanaIcon({ size = defaultSize }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="#F06A6A" aria-hidden>
...
function GongIcon({ size = defaultSize }: Props) {
  return (
