  ...
  const toolbar = (
    ...
    >
      ...
      <div ref={tableBtnRef} className="relative">
        ...
        {tableMenuOpen && inTable && (
          <TableMenu editor={editor} onClose={() => setTableMenuOpen(false)} />
        )}
      </div>
      <Btn
...
function TableMenu({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  function run(fn: () => void) {
    return () => {
      ...
      // moves the caret out). Close after every op so users don't fire
      // stale commands against a now-different cell.
      onClose();
    };
  }
  return (
  ...
  return (
    <button
      ...
      className={
        ...
        (destructive ? "text-red-600 hover:bg-red-500/10" : "text-foreground hover:bg-raised")
      }
    >
      {children}
    </button>
